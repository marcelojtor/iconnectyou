(async function () {
  ...
  const setText = (bind, val) => qsa(`[data-bind="${bind}"]`).forEach(el => el.textContent = val || '');
  ...
  const waDisplay = qs('#wa_display'); if (waDisplay) waDisplay.href = waLink;
  const waFloat = qs('#wa_float'); if (waFloat) waFloat.href = waLink;

  // Telefone / Email
  const tel = qs('#telefone'); if (tel) tel.textContent = cfg.contact.phone || '';
  const emailLink = qs('#email_link');
  if (emailLink) { emailLink.textContent = cfg.contact.email || ''; emailLink.href = `mailto:${cfg.contact.email || ''}`; }

  // Cards
  const cards = qs('#cards_container');
  if (cards && Array.isArray(cfg.cards)) {
    cards.innerHTML = cfg.cards.slice(0,3).map(c => `
      <article class="card">
  ...
  // Legal strip (CNPJ / razão / etc.)
  const legal = qs('#legal_lines');
  if (legal && cfg.legal_info?.lines?.length) {
    legal.innerHTML = cfg.legal_info.lines.map(line => `<span class="line">${line}</span>`).join('');
  }
  ...
  if (payWrap && Array.isArray(cfg.payments?.methods)) {
    payWrap.innerHTML = cfg.payments.methods.map(m => {
  ...
  if (certList && Array.isArray(cfg.certs?.items)) {
    certList.innerHTML = cfg.certs.items.map(txt => `
      <li>
  ...
  const end = qs('#endereco'); if (end) end.textContent = cfg.business.address || '';
  const hrs = qs('#horarios'); if (hrs && Array.isArray(cfg.business.hours)) hrs.innerHTML = cfg.business.hours.map(h=>`<div>${h}</div>`).join('');
  const map = qs('#map_iframe'); if (map && cfg.business.maps_embed) map.src = cfg.business.maps_embed;

  // Cor primária
  if (cfg.colors?.primary) document.documentElement.style.setProperty('--primary', cfg.colors.primary);

  // WhatsApp flutuante: aparece após 40px de scroll (e já visível no mobile)
  const toggleWA = () => {
    if (!waFloat) return;
